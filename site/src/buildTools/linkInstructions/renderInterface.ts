...
const handleLinkNode = ({
  ...
  ancestors[0].children.splice(
    ...
    {
      type: 'jsx',
      value: `<InterfaceRenderer__LinkInstructions data={${APIDataIdentiferName}} name="${instructionParam}" />`,
    }
  )
...
const afterVisitTree = (tree, file) => {
  ...
  tree.children.unshift({
    type: 'import',
    value: `import InterfaceRenderer__LinkInstructions from '@siteComp/APIDataRenderer/interface'`,
  })
}
