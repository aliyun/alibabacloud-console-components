...
const MenuEventEmitter = new SimpleEventEmitter();

const dispatchUpdatePath = (path: string) => {
  const realPath = path?.split?.('?')[0];

  if (realPath) MenuEventEmitter.emit('updatePath', realPath);
};

const normalizeItems = (
  items: IRoutableItemDescriptor[] = [],
): IItemDescriptor[] => items.map((item) => {
...
const isValidDescriptor = (
  descriptor: any,
): descriptor is { key: string; length: number; match: any } => typeof descriptor === 'object' &&
  !isNil(descriptor) &&
...
const useRoutableMenu = (
  ...
  // 0.
  const [activeKeyFromAnother, setActiveKeyFromAnother] = useState<string | null>(null);
  // activeKeyFromAnother 的副作用标记
  const anotherSign = useRef(0);
  ...
  // 2. 将pathname与item的key匹配，得到最接近的item
  const matchResult = useBuiltInLocationMatcher(anotherSign.current && activeKeyFromAnother ? activeKeyFromAnother : pathname, items) || {
    key: undefined,
    match: undefined,
  };
  ...
  useEffect(() => {
    const off = MenuEventEmitter.on('updatePath', (path: string) => {
      if (!path) return;

      setActiveKeyFromAnother((prePath) => {
        if (path !== prePath) {
          anotherSign.current = 1;
        }
  ...
  useEffect(() => {
    if (!activeKeyFromAnother) return;

    anotherSign.current = 0;
  }, [activeKeyFromAnother]);
