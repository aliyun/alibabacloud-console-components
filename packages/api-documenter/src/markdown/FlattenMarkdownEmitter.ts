...
export class FlattenMarkdownEmitter extends MarkdownEmitter {
  ...
  protected writeNode(
    ...
    switch (docNode.kind) {
      case CustomDocNodeKind.Heading: {
        ...
        const anchor: string = docHeading.anchorName
          ? `<a name="${docHeading.anchorName}"></a>`
          : '';

        writer.writeLine(
          prefix + ' ' + anchor + this.getEscapedText(docHeading.title)
        );
        writer.writeLine();
        break;
      }
